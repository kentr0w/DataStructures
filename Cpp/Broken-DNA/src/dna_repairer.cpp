...
void DNARepairer::repairDNA()
{
    ...
    Id2DnaMap map;
    NodeDNAChain* _nodednachain=_dnaStorage.getPreHead()->next;
    DNAChain* _dnachain = &_nodednachain->value;
    NodeDNA *_nodedna = _dnachain->getPreHead()->next;
     for(int j =0; j<_dnaStorage.size(); j++)
     {
        for (int i = 0; i<_nodednachain->value.size(); i++)
        {
            Id2DnaMap::iterator obj = map.find(_nodedna->value.id);
            if(obj==map.end())
            {
                DNAChain* nd= new DNAChain();
                nd->addElementToEnd(_nodedna->value);
                map.insert(pair<std::string, DNAChain*>(_nodedna->value.id, nd));
            }
            else
            {
                DNAChain* dn = obj->second;
                dn->addElementToEnd(_nodedna->value);
            }
            _nodedna=_nodedna->next;
        }
        if(_nodednachain->next== nullptr)
            break;
        _nodednachain=_nodednachain->next;
        _dnachain=&_nodednachain->value;
        _nodedna=_dnachain->getPreHead()->next;
    }


    ListOfDNAChains objec;
    for(auto q=map.begin(); q!=map.end(); q++)
    {
        DNAChain* object_1=q->second;
        for(int i=0; i<object_1->size()-1; i++)
        {
            for (int j = 0; j < object_1->size() - i-1; j++)
            {
                if (object_1->operator[](j).number>object_1->operator[](j+1).number)
                {
                    DNAElement tmp = object_1->operator[](j);
                    object_1->operator[](j) = object_1->operator[](j+1);
                    object_1->operator[](j+1) = tmp;
                }
            }
        }
        objec.addElementToEnd(*object_1);
    }
    _dnaStorage=objec;
}
